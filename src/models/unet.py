...
class FVDBUNetBaseUpsampler(torch.nn.Module):
    LAYERS = (2, 2, 2, 2, 2, 2, 2, 2)
    CHANNELS = (32, 64, 128, 256, 256, 128, 96, 96)
    INIT_DIM = 32
    OUT_TENSOR_STRIDE = 1
    ...
    def __init__(self, in_channels, out_channels, D=3):
        ...
        self.final = fvnn.SparseConv3d(self.CHANNELS[7], self.CHANNELS[7], kernel_size=3, stride=2, transposed=True)
        self.final2 = fvnn.SparseConv3d(self.CHANNELS[7], out_channels, kernel_size=1)
        self.relu = fvnn.ReLU(inplace=True)
    ...
    def forward(self, x, out_grid):
        ...
        out = self.final(out, out_grid=out_grid)
        out = self.final2(out)
        return out