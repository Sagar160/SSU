...
if __name__ == "__main__":
    input_dir = '/data/workspaces/spanwar/dataset/rebuttal/noise_meshes2/'
    ...
    for case in cases:
        ...
        if case.startswith('.'):
            continue
        input_dir_case = os.path.join(input_dir, case)
        filenames = [
            f'{f}.obj' for f in water_filenames]
        filenames_obj = [f for f in water_filenames]

        out = joblib.Parallel(n_jobs=-1)(
                joblib.delayed(run_eval)(file, file_obj, input_dir_case, gt_dir)for file, file_obj in zip(filenames, filenames_obj)
            )

        print(f'number of samples for case {case}, method: ponq:', len(out))
        out = np.array(out)
        ...
        print(f"case {case}, method: ponq CD1 (x 1e-5): {cd1*1e5:.3f} CD2 (x 1e-5): {cd2*1e5:.3f} F1: {f1:.3f} NC: {nc:.3f} ECD2: {ecd2*1e2:.3f} EF1: {ef1:.3f}")
