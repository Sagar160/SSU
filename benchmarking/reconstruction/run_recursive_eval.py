...
def run_eval(filename, filename_obj, input_dir, gt_dir):
    # load pred mesh
    input_path = os.path.join(input_dir, f'{filename.split(".")[0]}_recursive.obj')
    pred_mesh = trimesh.load(input_path, force='mesh')
    v = pred_mesh.vertices
    f = pred_mesh.faces
    # v = (v-0.5)*2

    # v = NDCnormalize(v, return_scale=False)+0.5
    ...
    gt_mesh = trimesh.Trimesh(vertices=gt_v, faces=gt_mesh.faces)
    print(v.min()-0.5, v.max()-0.5, gt_v.min()/2, gt_v.max()/2)
...
if __name__ == "__main__":
    input_dir = '/data/workspaces/spanwar/results/ssu/rebuttal/recursive'
