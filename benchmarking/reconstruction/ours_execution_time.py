...
def prepare_all_inputs(sdf_numpy, grid_size, upsample_factor=4):
    ...
    new_features = get_all_shifted_positions(
        input_vdb, grid_size, upsample_factor)
    all_inputs = []
...
def get_prediction(all_inputs, grid_size, upsample_factor, model, BATCH_SIZE=20):
    ...
    pred_ijk = (pred_list.ijk.jdata)*upsample_factor + \
        (vector*(upsample_factor//2)).long()

    # prepare output grid
    output_grid = fvdb.gridbatch_from_ijk(pred_ijk)
    output_vdb = fvnn.VDBTensor(output_grid, output_grid.jagged_like(
        torch.zeros(output_grid.num_voxels, device=device)))
...
def ours_execution_time():
    ...
    for res in [32, 64, 128]:
        for filename in tqdm(filenames):
            ...
            output_vdb = get_prediction(
                all_inputs, res+1, 4, model, BATCH_SIZE=20)
            # Reconstruct triangle mesh
            Vr, Fr = vdb_marching_cubes(output_vdb)
            end_time = time.time()
