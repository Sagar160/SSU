...
def main(config_file, input_dir, s):
    clear_gpu_memory()
    ...
    data_loader = DataLoader(input_dir=input_dir, 
    ...
    model = torch.load(config['model']['save_model_path'])
    model = model.to('cuda' if torch.cuda.is_available() else 'cpu')

    save_predictions_dir = config['prediction']['save_predictions_dir']
    prediction_folder_name = config['prediction']['prediction_folder_name'] + f'{s}'
...
if __name__ == "__main__":
    ...
    for s in [0.25]:
        input_dir = f'/data/workspaces/spanwar/results/ssu/rebuttal/our_vs_rfta/mesh/s_{s}'
        # file =  input_dir + '/527631.hdf5'
        # # check the keys
        # with h5py.File(file, 'r') as f:
        #     print(f"Keys in {file}: {list(f.keys())}")
        main(config_file, input_dir, s)