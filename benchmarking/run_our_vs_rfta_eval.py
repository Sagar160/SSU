...
def run_eval(filename, filename_obj, input_dir, gt_dir, a):
    ...
    # print(v.min()-0.5, v.max()-0.5, 'at scale:', a)
    # print(gt_mesh.vertices.min()/2, gt_mesh.vertices.max()/2)
...
if __name__ == "__main__":
    ...
    for a in [0.25]:
        ...
        for size in [32, 64, 128]:
            ...
            filenames = [
                f'{size}_{f}.obj' for f in water_filenames]
            ...
            print(f"size: {size} method: our_vs_rfta CD1 (x 1e-5): {cd1*1e5:.3f} CD2 (x 1e-5): {cd2*1e5:.3f} F1: {f1:.3f} NC: {nc:.3f} ECD2: {ecd2*1e2:.3f} EF1: {ef1:.3f}")