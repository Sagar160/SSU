...
def main(config_file):
    ...
    if not config['eval']['only_eval']:
        if config['training']['use_pre_train_model']:
        ...
        if config['training']['optimizer']=='SGD':
            ...
            warmup = torch.optim.lr_scheduler.LinearLR(optimizer, start_factor=(3e-5/3e-4), total_iters=201)  # 1 epoch
        ...
        trainer = model_training.ModelTrainer(
                                ...
                                sdf_scaling=config['data']['sdf_scaling_value'],
                                optimizer=optimizer,
                                scheduler=scheduler,
